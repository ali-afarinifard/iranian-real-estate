      ...
      >
        ...
        <Box sx={{ position: 'relative', height: 140 }}>
          ...
          <Typography
            variant="h6"
            fontWeight={800}
            sx={{ position: 'absolute', bottom: 10, left: 12, color: '#fff', lineHeight: 1.1 }}
          >
            {formatPrice(property.price, property.currency as 'EUR')}
            {!isSale && (
              <Typography component="span" variant="caption" sx={{ color: 'rgba(255,255,255,0.8)', ml: 0.5 }}>
                /mo
              </Typography>
            )}
          </Typography>
          ...
          <Stack direction="row" spacing={1.5} alignItems="center">
            {property.bedrooms > 0 && (
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.4 }}>
                ...
                <Typography variant="caption" fontWeight={600}>{property.bedrooms}</Typography>
              </Box>
            )}
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.4 }}>
              <SquareFootRounded sx={{ fontSize: 14, color: 'text.secondary' }} />
              <Typography variant="caption" fontWeight={600}>{formatArea(property.area)}</Typography>
            </Box>
